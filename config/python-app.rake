...
task :check_required_tools do
    ...
    $required_commands | ['nginx', 'pip', 'virtualenv'] # union these arrays
    ...
    if not is_python_dev_installed? then
        puts "ERROR: python-dev not installed, speedups won't be enabled." 
        exit 1
    else
...
task :restart_app do
    ...
    ssh("cd #{$root}; source #{$virtualenv_dir}/bin/activate; sh -c \"nohup python #{app} > nohup.out 2>&1 &\"")
