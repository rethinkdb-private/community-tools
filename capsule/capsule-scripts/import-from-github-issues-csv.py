...
MAX_NOTE_LENGTH = 16000 # Notes can be a maximum of 16,000 characters long
QUIET = True # Verbose or quiet mode
...
def add_data(resource, data):
    ...
    if not req.status_code in [200, 201]:
        print "\nHTTP %d when adding Capsule data for resource %s\nData:%s\nResponse:%s" % (req.status_code, resource, json.dumps(data), req.text)
    return req
def add_tag(party_id, tag, name):
    req = requests.post(CAPSULE_SITE+'/api/party/'+party_id+'/tag/'+urllib.quote_plus(tag), auth=auth, headers=headers)
    if not req.status_code in [200, 201]:
        print '\nError adding tag %s to %s.' % (tag, name)
    return req
def set_custom_field(party_id, data, name):
    req = requests.put(CAPSULE_SITE+'/api/party/'+party_id+'/customfields', data=json.dumps(data), auth=auth, headers=headers)
    if not req.status_code in [200, 201]:
        print '\nError updating custom fields for %s.' % (name,)
    return req
def get_matching_user(github_login):
    req = requests.get(CAPSULE_SITE+'/api/party?q='+github_login, auth=auth, headers=headers)
    if req.status_code is not 200:
        print "\nHTTP %d when looking up user details on Capsule for %s" % (req.status_code, github_login)
...
def update_user(user_id, user_data, name):
    ...
    if req.status_code is not 200:
        print "\nHTTP %d when updating Capsule user %s (%s): %s : %s" % (req.status_code, user_id, name, url, req.text)
...
def add_user_to_capsule(user):
    ...
    if gh_user['email']:
        if not re.match(r"[^@]+@[^@]+\.[^@]+", gh_user['email']):
            if not QUIET:
                print "\nSkipping bad email for %s: %s" % (gh_user['login'], gh_user['email'])
            errors['bad_email'] = {
                'login': gh_user['login'],
                'data': gh_user,
                'error': "Skipped bad email for %s: %s" % (gh_user['login'], gh_user['email']),
            }
        else:
    ...
    if existing_user is None:
        if not QUIET:
            print "\nAdding user %s that doesn't exist in Capsule" % gh_user['login']
        # Create the new user on Capsule
        resource = '/api/person'
        req = add_data(resource, new_user)
        if req and req.headers:
            person_id = req.headers['location'].rsplit('/',1)[1]
        else:
            if not QUIET:
                print "\nSkipping user %s because of an error." % gh_user['login']
            errors['capsule_new'] = {
                'login': gh_user['login'],
                'endpoint': resource,
                'data': new_user,
                'error': "Skipped adding events for user %s because of an error." % gh_user['login'],
                'response': req.text, 
            }
            return
    # The user exists, so resolve conflicts and update the user in Capsule
        ...
        if len(merged_user['person']) > 0:
            if not QUIET:
                print "\nMerged user %s and adding to Capsule" % gh_user['login']
            req = update_user(person_id, merged_user, name)
            if req.status_code is not 200:
                errors['capsule_existing'] = {
                    'login': gh_user['login'],
                    'endpoint': '/api/party/'+person_id,
                    'data': merged_user,
                    'error': "Skipped adding events for existing user %s because of an error." % gh_user['login'],
                    'response': req.text, 
                }
                return

    # Set the custom fields for the person we just created: update their GitHub custom field
    custom_field_data = {
        'customFields': {
            'customField': [
                {
    ...
    req = set_custom_field(person_id, custom_field_data, name)
    if req.status_code not in [200,201]:
        errors['set_custom_field_gh'] = ({
            'login': gh_user['login'],
            'endpoint': '/api/party/'+person_id+'/customfields/',
            'data': custom_field_data,
            'error': "Error setting __github custom field for %s." % gh_user['login'],
            'response': req.text, 
        })
    ...
    for tag in [UNREVIEWED_TAG, IMPORTED_TAG, SOURCE_TAG]:
        ...
        if req.status_code not in [201,200]:
            tag_errors.append({
                'login': gh_user['login'],
                'endpoint': '/api/party/'+person_id+'/tag/',
                'data': tag,
                'error': "Error adding tag %s for %s." % (tag, gh_user['login']),
                'response': req.text, 
            })

    # If we ran into any errors while adding tags, note them
    if len(tag_errors) > 0:
    ...
    for event in events:
        if not QUIET:
            print "\nAdding event by %s: %s on %s" % (gh_user['login'], event['event'], event['timestamp'])
        ...
        history_data = {
            'historyItem': {
                ...
                'note': note[:MAX_NOTE_LENGTH - 3] + '...' if len(note) > MAX_NOTE_LENGTH else note
            }
        }
        ...
        if req.status_code not in [200,201]:
            event_errors.append({
                'login': gh_user['login'],
                'endpoint': '/api/party/'+person_id+'/history/',
                'data': history_data,
                'error': "Error adding history note for %s." % gh_user['login'],
                'response': req.text, 
            })
    if len(event_errors) > 0:
        errors['event_errors'] = event_errors

    if len(errors) > 0:
        return { gh_user['login']: errors }
    return None
...
def merge_user(existing_user, new_user):
    if not existing_user.get('person') or not new_user.get('person'):
        print "\nNew or existing user is missing a top-level 'person' field:\n\nExisting user: %s\n\nNew user: %s" % (existing_user, new_user)
        return
...
def start_import():
    ...
    p = Pool(10)
    error_set = p.map(add_user_to_capsule, users.values())


    # Flatten the error set (remove None types and build a dict with key value pairs for users and errors).
    errors = {}
    for e in error_set:
        if e:
            errors.update(e)
