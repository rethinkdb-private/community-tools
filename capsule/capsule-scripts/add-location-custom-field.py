...
def start():
    parties = {}
    print "Fetching list of users in Capsule..."
    ...
    for user in users:
        ...
        if type(user['contacts']) is not dict:
            continue
        try:
            addresses = user['contacts']['address']
            # If there's one website, it's a dict, otherwise, it's a list of dicts
            if len(addresses) == 0:
                continue
            elif type(addresses) is dict:
                addresses = [addresses]
            if addresses[0]['city']:
                user_map.append([user['id'], addresses[0]['city']])
        except KeyError:
            continue

    print "Updating locations for %d users." % len(user_map)
